  ...
  return (
    <Card className="max-w-[800px] min-w-[350px]">
      ...
      <CardContent className=" max-w-[95%] md:max-w-[90%] mx-auto">
        <form>
          {formErr && <div className="err-message text-center text-[red] py-2">{formErr}</div>}
          <div className="grid w-full items-center gap-8">
            <div className="flex flex-col space-y-2.5">
              <Label htmlFor="name" className="">
                Email/Username
              </Label>
              <InputCont id="email" name='email' onChange={handleChange} placeholder="Enter your email/username"></InputCont>
            </div>

            <div className="flex flex-col space-y-2.5">
              <Label htmlFor="password">Password</Label>
              <div className="flex justify-between items-center">
              <InputCont id="password" name='password' passwordVisible={passwordVisible} onChange={handleChange} placeholder="Enter your password" className={'justify-between items-center'}>
                {
                  passwordVisible ? <FaEyeSlash size={28} onClick={() => setPasswordVisible(false)} /> : <FaEye size={28} onClick={() => setPasswordVisible(true)} />
                }
      ...
      <CardFooter className="max-w-[90%] mx-auto py-10">
        <Button onClick={handleSubmit} size="full">Login</Button>
      </CardFooter>
    </Card>
  );
}
