  ...
  return (
    <nav className="absolute inset-x-0 flex justify-center items-center py-10 text-xl 2xl:text-[28px] z-[100]">
      <div className="w-[98%] grid grid-flow-col items-center">
        ...
        <Link href={'/home'} className="absolute flex flex-col items-center text-[32px]/[3rem] 2xl:text-[40px] text uppercase w-[40vw] text-center font-bold text-primaryBlue justify-self-center left-[50%] -translate-x-[50%] top-[50%] -translate-y-[50%]">
        ...
        <Menubar className='justify-self-end'>
          <MenubarMenu>
            {
              navItems.map((item, i) => {
                return (<MenubarTrigger className={`${item == 'Search' ? 'text-oauOrange' : ''}`}>{item}</MenubarTrigger>)
              })
            }
